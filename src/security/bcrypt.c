/* bcrypt.c — portable bcrypt verification (verify direction only).
 *
 * Adapted from OpenBSD's bcrypt.c / blowfish.c (Copyright (c) 1997 Niels
 * Provos <provos@umich.edu>, 2014 Ted Unangst <tedu@openbsd.org>, and
 * David Mazieres <dm@lcs.mit.edu>; ISC-style license, quoted below), with
 * the Blowfish tables initialized with digits of Pi.  Only the verify
 * direction is implemented: agent-httpd never creates hashes, it checks
 * Authorization headers against a pre-existing htpasswd file.
 *
 * License (OpenBSD):
...
#define BCRYPT_WORDS 6        /* 64-bit ciphertext as 6 32-bit words */
#define BCRYPT_MAXSALT 16     /* 128-bit salt */

typedef struct {
    uint32_t S[4][256];   /* S-Boxes come first: matches the OpenBSD
                             initstate literal order */
    uint32_t P[18];       /* Subkeys */
} blf_ctx;

/* ------------------------------------------------------------------ */
/* Blowfish primitives (adapted verbatim from OpenBSD blowfish.c)      */
/* ------------------------------------------------------------------ */
...
Blowfish_encipher(blf_ctx *c, uint32_t *xl, uint32_t *xr)
{
    ...
    Xl ^= p[0];
#define BLFRND(s, p, i, j, n) (i ^= ((((s)[(((j) >> 24) & 0xFF)] + (s)[0x100 + (((j) >> 16) & 0xFF)]) ^ (s)[0x200 + (((j) >> 8) & 0xFF)]) + (s)[0x300 + ((j) & 0xFF)]) ^ (p)[n])
...
/* ------------------------------------------------------------------ */
/* bcrypt base64 (adapted from OpenBSD bcrypt.c)                       */
/* ------------------------------------------------------------------ */
...
#define CHAR64(c)  ((c) > 127 ? 255 : index_64[(uint8_t)(c)])
...
int
bcrypt_is_hash(const char *stored)
{
    if (!stored)
        return 0;
...
/* Constant-time equality of two buffers of length n. */
static int
timingsafe_eq(const char *a, const char *b, size_t n)
{
...
int
bcrypt_verify(const char *pass, const char *stored)
{
    blf_ctx state;
    size_t key_len;
    uint32_t rounds, k, i;
    ...
    uint8_t ciphertext[4 * BCRYPT_WORDS];
    uint8_t csalt[BCRYPT_MAXSALT];
    uint32_t cdata[BCRYPT_WORDS];
    char expected[7 + 22 + 31 + 1]; /* "$2?$cc$" + salt22 + hash31 + NUL */
    const char *orig = stored;

    if (!pass || !stored || !bcrypt_is_hash(stored))
        return 0;

    /* "OrpheanBeholderScryDoubt" — exactly 24 bytes, no NUL */
    ...
    switch ((minor = stored[1])) {
    case 'a':
        /* include the NUL; no 72-byte truncation (legacy) */
        key_len = strlen(pass) + 1;
        break;
    case 'b':
        /* cap at 72 bytes, then include the NUL */
        key_len = strlen(pass);
        if (key_len > 72)
            key_len = 72;
        key_len++;
    ...
    case 'y':
    case 'x':
        /* crypt_blowfish variants; for ASCII passwords identical to 'a' */
        key_len = strlen(pass) + 1;
        break;
    default:
        return 0;
    ...
    /* Parse rounds: exactly two digits */
    if (stored[0] < '0' || stored[0] > '9' ||
        stored[1] < '0' || stored[1] > '9' || stored[2] != '$')
        return 0;
    logr = (uint8_t)((stored[0] - '0') * 10 + (stored[1] - '0'));
    if (logr < 4 || logr > 31)
        return 0;
    rounds = 1U << logr;

    stored += 3; /* now at the 22-char salt */

    if (strlen(stored) * 3 / 4 < BCRYPT_MAXSALT)
        return 0;
    if (decode_base64(csalt, BCRYPT_MAXSALT, stored) != 0)
        return 0;
    salt_len = BCRYPT_MAXSALT;

    /* Setting up S-Boxes and Subkeys (EksBlowfish) */
    Blowfish_initstate(&state);
    Blowfish_expandstate(&state, csalt, salt_len,
        (const uint8_t *)pass, (uint16_t)key_len);
    for (k = 0; k < rounds; k++) {
        Blowfish_expand0state(&state, (const uint8_t *)pass, (uint16_t)key_len);
        Blowfish_expand0state(&state, csalt, salt_len);
    }

    /* Build the 6 ciphertext words from "OrpheanBeholderScryDoubt" */
    j = 0;
    for (i = 0; i < BCRYPT_WORDS; i++)
        cdata[i] = Blowfish_stream2word(ciphertext, 4 * BCRYPT_WORDS, &j);

    /* Now do the encryption (64 rounds over 3 blocks) */
    for (k = 0; k < 64; k++)
    ...
    /* Serialize the expected full hash string */
    snprintf(expected, sizeof(expected), "$2%c$%2.2u$", minor, logr);
    encode_base64(expected + 7, csalt, BCRYPT_MAXSALT);
    encode_base64(expected + 7 + 22, ciphertext, 4 * BCRYPT_WORDS - 1);

    /* Constant-time compare against the original stored hash */
    if (strlen(expected) != strlen(orig) ||
        !timingsafe_eq(expected, orig, strlen(orig)))
        return 0;
