#!/usr/bin/env bash
# One-off init for the in-container MCP bridges.
#
...
# has to be populated once; one project in particular runs `uv run --no-sync`, so
...
sync_one() {
  ...
  # 被桥驱动的 Python 项目依赖树很重 (pandas/numpy/scipy/xgboost/lightgbm … 未缓存时
