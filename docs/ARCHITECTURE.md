...
```
                     ...
                     │              agent-httpd 进程                │
                     │                                            │
   accept() ───────► │  master 事件循环 (src/event.c)              │
                     │   │ 快路径直服 (不 fork):                   │
                     ...
                     │   ▼                                        │
                     │  prefork worker 池 × 8 (src/worker.c)       │
   ...
   (src/cgi.c)       (src/agent.c)     dev: Vite           (src/fastcgi.c)
...
入口 `src/main.c`。启动顺序有讲究——**所有共享状态的初始化必须在 fork 之前**，
让 worker 继承而非重建：
...
main()
 ...
 ├─ skills_init()          # SKILL.md 扫描 (main.c:191)
 ├─ tools_init()           # 内置 7 工具注册 (main.c:192)
 ├─ mcp_init()             # 每个 MCP server 起一次性子进程做 tools/list (main.c:193)
 ├─ create_server_socket() + 可选 create_fastcgi_listener(-F)
...
### 1.1 master 事件循环（`src/event.c`）

- 单进程 kqueue（macOS）/ epoll（Linux），accept 所有连接。
- **`recv(MSG_PEEK)` 零消费预览**：只偷看请求头判断快/慢，不消费字节。慢请求
  交给 worker 时头字节仍在内核缓冲，worker 的 `handle_client` 像自己 accept 的
  一样直接重读——不需要主循环→worker 的字节重放（`src/event.c:9-12`）。
- 快路径判定在 `is_fast_request()`（`src/http.c:272`）：GET/HEAD/OPTIONS、
  无 body、非 CGI、非 chat 路由、非 `-v` 代理路由。
- 快路径内部响应（错误页/默认 Content-Type 等）由 `event.c:177` 处的序列化
  逻辑生成，慢路径分派时丢弃重建。

### 1.2 prefork worker 池（`src/worker.c`）
...
- **并发上限 = 管道计数信号量**：管道初始放 N 个 token 字节（= 空闲 worker 数）；
  ...
  上避开未实现的 unnamed POSIX sem（`src/worker.c:1-15` 注释）。
- **优雅排水**：SIGTERM → worker 设 `g_shutdown_requested` → 完成手头连接
  （keep-alive 循环见到标志即关）→ 父进程排水最多 5s → SIGKILL 兜底。

### 1.3 fork-per-connection（`-w 0`）

经典模型，`src/main.c:233-335` 的 select 循环。保留原因：教学对照 + 池化
故障时的回退路径（`main.c:216` 启动失败自动降级）。

## 2. HTTP 核心（`src/http.c`）

一次请求的完整生命周期（含可观测埋点 `src/metrics.c`——状态码分类在
...
recv 头 → parse_request (行/头/字长校验, 431 防护)
       ...
       → Expect: 100-continue? 即时下发 100 (http.c:675-684)
       → 读 body (与 Method 无关, 只看 content_length; 413/408 防护)
       → process_request (http.c:329)
            │ 方法分发 (见 §3)
            ├─ CGI 路径 → execute_cgi
...
- **`process_request()` 是唯一分发核心**（`src/http.c:329`）：HTTP 模式、
  FCGI 服务端模式（`fastcgi.c:510` 重建请求后调用同一个函数）、worker 池、
  fork 模式全部收敛到这一条链——四处路径一份语义。
- **`is_fast_request()` / `is_vite_proxy_route()`** 是事件循环的两个路由谓词，
  在头解析后、process_request 前调用。
- **`build_response()`**（`src/http.c:185`）统一序列化：每响应必带 Date（RFC
  9110 6.6.1）、安全头三件套（nosniff/DENY/no-referrer）、`Server: AgentHTTPD`
  固定指纹。`Allow` 头（405/OPTIONS）走 `response->allow`。

## 3. 方法分发的语义分层（`src/http.c:348-383`）

```
GET/HEAD          → 读链 (静态 or CGI)
POST/PUT/PATCH/DELETE → 仅 CGI 路径放行 (REQUEST_METHOD 透传, src/cgi.c:78)
...
## 4. 静态文件链（`src/static.c`）

```
resolve_within (realpath + .. 逃逸防护, static.c:37)
 → MIME (util.c 扩展名表)
...
浏览器 ──GET /react/?name=A──► agent-httpd :PORT (唯一公网入口)
  ...
  │  HMR WebSocket = TCP 隧道 (is_websocket_upgrade, http.c:307)
  ▼
...
2. **`/react/api/chat` 永不被代理**：`is_vite_proxy_route` 显式排除
   （`src/http.c:300`），chat 始终由 C 原生处理。
...
| 模块 | 职责 |
|---|---|
| `src/llm.c` | **端点壳**：路由谓词、`{message, history?, sessionId?}` 解析、`.env` 的 `LLM_*` 加载（环境变量优先）、离线演示引擎 |
| `src/agent.c` | **ReAct 主循环**：fork curl 上游、tool_calls 流式分片累积、每轮独立 fork、并发槽、上游重试/退避 |
| `src/chatio.c` | **SSE 信封**：note/delta/error/done 四事件 + 捕获缓冲 |
| `src/minijson.c` | sbuf + 容错 JSON 读取器（\uXXXX/代理对/嵌套跳过） |

请求路由（`llm_handle_chat`）：

```
有 LLM_API_KEY?
 ├─ PSE_ENABLED=true  → pse_run (src/pse.c): Planner(无工具出计划)
...
**并发槽**（`agent.h:1-30` 注释）：agent 一跑可能几分钟，占住 worker。管道
信号量限制同时在跑的 agent 数；满槽时 fail-fast 返回 "busy" 错误事件——
保护 worker 池不被长会话饿死。PSE 全场算 1 个槽。

**上游瞬时故障重试**（`agent.c:550` `agent_retryable` + 586 重试循环）：
...
幂等性保证在 `agent_run_inner`（`agent.c:586` 注释）：**messages 只在整轮成功
...
| 层 | 实现 | 说明 |
|---|---|---|
| 内置 7 工具 | `src/tools.c` | get_time/calc/read_file(fetch 16KB)/skill-run/remember/recall；`tools_dispatch` 统一分发 |
| Skills 索引 | `src/skills.c` | SKILL.md frontmatter 扫描 → 注入 system 提示；目录链 HARNESS_SKILLS_DIR → ./skills → resolve-skills/skills → SKILLS_EXTRA_DIRS |
| Memory | `src/session.c` | `.data/sessions/<id>.json` `{messages[], facts{}}`，tmp+rename 原子写；转录回放构成 history；30 天自动清理 |
| MCP | `src/mcp.c` | stdio JSON-RPC；启动 spawn 一次做 initialize+tools/list（pre-fork，worker 继承活代理）；每次 tools/call 起一次性子进程，不泄漏 fd/pid |
...
1. `src/event.c` + `src/worker.c` —— 进程模型与快慢分流
2. `src/http.c` 的 `handle_client`/`process_request` —— 请求生命周期
3. `src/static.c` + `src/cgi.c` —— 两条分发链
4. `src/agent.c` 头部 + `agent_run_inner` —— ReAct 循环与重试/并发
5. `src/llm.c` 的 `llm_handle_chat` —— chat 端点路由
6. `src/mcp.c` 的 `mcp_config_load` + 两层生命周期 —— MCP 桥
